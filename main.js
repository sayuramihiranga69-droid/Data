...
const responsee = await axios.get('https://raw.githubusercontent.com/Data-LK/database/refs/heads/main/main_var.json');
    ...
    if (connection === 'close') {
        ...
        setTimeout(async () => {
            try {
                ...
                try {
                    const response = await axios.get('https://raw.githubusercontent.com/Data-LK/database/refs/heads/main/main_var.json');
...
*\`• Logo :\`* ${logo ?? "https://files.catbox.moe/ccmrqx.jpg"}
*\`• Anti Delete :\`* ${antiDelete ?? "off"}
*\`• Leave Msg :\`* ${leaveMsg || "None"}
`;


     let joinlink2 = await fetchJson('https://raw.githubusercontent.com/Data-LK/database/refs/heads/main/main_var.json');
...
const ownerdataa = (await axios.get('https://raw.githubusercontent.com/Data-LK/database/refs/heads/main/main_var.json')).data;
...
const ownerdata = (await axios.get('https://raw.githubusercontent.com/Data-LK/database/refs/heads/main/main_var.json')).data
            
           
            config.FOOTER = ownerdata.footer
           
const preUser = await fetchJson(`https://raw.githubusercontent.com/Data-LK/database/refs/heads/main/prime_users.json`)
...
const rec = (await axios.get('https://raw.githubusercontent.com/Data-LK/database/refs/heads/main/react.json')).data

const recc = (await axios.get('https://raw.githubusercontent.com/Data-LK/database/refs/heads/main/main_var.json')).data
...
if(!isOwner) {	
    ...
    //await conn.sendMessage(from, { audio: fs.readFileSync("./" + type.ext), mimetype:  originalMessage.message.audioMessage.mimetype, fileName:  `${m.id}.mp3` })	
     const sdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'RAVANA-X-MD 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: sdata});
...
app.get("/", (req, res) => {
  res.send("📟 RAVANA DL Working successfully!");
});
app.listen(port, () => console.log(`Movie-Ravan-Md Server listening on port http://localhost:${port}`));
setTimeout(() => {
connectToWA()
}, 3000);
